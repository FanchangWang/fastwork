 ...
 * @see \Core\Validate
 * @mixin \Core\Validate
 * @method \Core\Validate make(array $rules = [], array $message = [], array $field = []) static 创建一个验证器类
 * @method \Core\Validate rule(mixed $name, mixed $rule = '') static 添加字段验证规则
 ...
 * @method \Core\Validate message(mixed $name, string $message = '') static 设置提示信息
 * @method \Core\Validate scene(string $name) static 设置验证场景
 * @method bool hasScene(string $name) static 判断是否存在某个验证场景
 * @method \Core\Validate batch(bool $batch = true) static 设置批量验证
 * @method \Core\Validate only(array $fields) static 指定需要验证的字段列表
 * @method \Core\Validate remove(mixed $field, mixed $rule = true) static 移除某个字段的验证规则
 * @method \Core\Validate append(mixed $field, mixed $rule = null) static 追加某个字段的验证规则
